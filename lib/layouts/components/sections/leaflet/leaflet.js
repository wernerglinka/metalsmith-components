...
const initLeafletMaps = () => {
  ...
  const createLeafletMap = (mapContainer) => {
    ...
    // Create custom icon (optional - comment out to use default)
    ...
    markers.forEach((marker) => {
      ...
      // Use custom icon if defined, otherwise use default
      const markerOptions = { title: markerTitle };
      // if (customIcon) markerOptions.icon = customIcon;
      
      L.marker([thisLatitute, thisLongitude], markerOptions)
        .addTo(leafletMap)
