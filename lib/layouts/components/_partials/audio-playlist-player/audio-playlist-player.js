...
class AudioPlaylist {
  ...
  bindEvents() {
    ...
    // Add keyboard navigation for track buttons
    this.container.addEventListener('keydown', (e) => {
      const trackButton = e.target.closest('[data-track-button]');
      if (!trackButton) return;

      const trackItem = trackButton.closest('.track-item');
      const allTrackButtons = Array.from(this.container.querySelectorAll('[data-track-button]'));
      const currentIndex = allTrackButtons.indexOf(trackButton);
      ...
      if (e.key === 'Enter' || e.key === ' ') {
        ...
        e.preventDefault();
        const nextIndex = (currentIndex + 1) % allTrackButtons.length;
        allTrackButtons[nextIndex].focus();
      } else if (e.key === 'ArrowUp') {
        e.preventDefault();
        const prevIndex = (currentIndex - 1 + allTrackButtons.length) % allTrackButtons.length;
        allTrackButtons[prevIndex].focus();
      } else if (e.key === 'Home') {
        e.preventDefault();
        allTrackButtons[0].focus();
      } else if (e.key === 'End') {
        e.preventDefault();
        allTrackButtons[allTrackButtons.length - 1].focus();
      }
    });
  }
  ...
  selectTrack(trackItem) {
    ...
    try {
      ...
      // Announce track selection to screen readers
      const trackTitle = this.currentTrack.title || 'Track';
      const trackArtist = this.currentTrack.artist || '';
      const announcement = `Now playing: ${trackTitle}${trackArtist ? ` by ${trackArtist}` : ''}`;
