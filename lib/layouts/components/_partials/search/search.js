...
async function initSearch() {
  try {
    ...
    // Initialize Fuse.js with the loaded index - optimized to minimize false positives
    const fuseOptions = {
      keys: [
        { name: 'title', weight: 10 },      // Titles are most precise
        { name: 'pageName', weight: 8 },    // Page names are curated
        { name: 'tags', weight: 7 },        // Tags are intentional keywords
        { name: 'leadIn', weight: 5 },      // Lead-ins are crafted content
        { name: 'prose', weight: 3 },       // Prose is editorial content
        { name: 'content', weight: 1 }      // Full content gets lowest weight
      ],
      threshold: 0.3, // Allow reasonable fuzzy matching
      includeScore: true,
      includeMatches: true,
      minMatchCharLength: 4, // Require substantial matches
      ignoreLocation: true, // Focus on match quality, not position
      findAllMatches: false,
      useExtendedSearch: true, // Enable extended search for better control
...
function handleSearch( event ) {
  ...
  // STEP 1: Perform fuzzy search to find potential matches
  let results = fuse.search( query );

  // STEP 2: Filter results to keep only those with actual substring matches
...
function displayResults( results, query ) {
  ...
  const resultsHTML = results.map( result => {
    ...
    // Add highlight parameter to URL (handle hash fragments correctly)
    ...
    return `
      <div class="search-result">
        ${ item.pageName && item.type === 'section' ? `<h3><a href="${ highlightUrl }" data-search-result>${ item.pageName }</a></h3>` : '' }
