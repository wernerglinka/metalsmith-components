...
function generateBundleReadme(version, components) {
  ...
  readme += `\`\`\`bash\n`;
  readme += `cp metalsmith-components.config.json ../\n`;
  readme += `\`\`\`\n\n`;
  ...
  readme += `\`\`\`bash\n`;
  readme += `./install-all.sh --update-only\n`;
  readme += `# or use the short form:\n`;
  readme += `./install-all.sh -u\n`;
  readme += `\`\`\`\n\n`;
...
function generateBundleInstallScript(components) {
  ...
  components.partials.forEach(partial => {
    ...
    script += `  if [ "$MODE" = "update" ] && [ ! -d "$COMPONENTS_BASE/$PARTIALS_DIR/${partial.name}" ]; then\n`;
  ...
  components.sections.forEach(section => {
    ...
    script += `  if [ "$MODE" = "update" ] && [ ! -d "$COMPONENTS_BASE/$SECTIONS_DIR/${section.name}" ]; then\n`;
