...
async function loadExamples(name, projectRoot, examplesPath, type) {
  ...
  try {
    const examplesContent = await fs.readFile(componentExamplesFile, 'utf-8');
    const examples = yaml.load(examplesContent);
    // Wrap in array with metadata if it's a raw config
    if (Array.isArray(examples) && examples[0]?.sectionType) {
      return examples.map((config, index) => ({
...
function generateInstallScript(component) {
  ...
  script += `# Cleanup extracted component directory if not called from bundle\n`;
  script += `if [ -z "$BUNDLE_INSTALL" ] && [ -f "$COMPONENT_DIR/package.json" ] && grep -q "\\"x-metalsmith-component\\"" "$COMPONENT_DIR/package.json" 2>/dev/null; then\n`;
